...
export default function ManageCategories({ onClose }: OnCloseProps) {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div>
            ...
            <select
              ...
              style={{ ...inputStyle, borderColor: focusedField === "type" ? C.borderFoc : C.border, colorScheme: "dark" }}
            >
          ...
          <div>
            ...
            <input
              type="text" name="name" placeholder="e.g. Salary, Food & Beverage"
  ...
  if (step === "list") {
        ...
        {userRole === 1 && (
          <td style={{ padding: "0.6rem 0.75rem", whiteSpace: "nowrap" }}>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <button onClick={() => handleOpenEdit(cat)} style={{ padding: "0.25rem 0.6rem", borderRadius: "0.375rem", border: `1px solid ${C.border}`, background: "transparent", color: C.primary, fontSize: "0.75rem", fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: "0.25rem" }}>
                <Pencil style={{ width: "0.7rem", height: "0.7rem" }} /> Edit
              </button>
              <button onClick={() => handleOpenDelete(cat)} style={{ padding: "0.25rem 0.6rem", borderRadius: "0.375rem", border: `1px solid ${C.expense}60`, background: "transparent", color: C.expense, fontSize: "0.75rem", fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: "0.25rem" }}>
    ...
    const SectionTable = ({ items, type }: { items: CategoryRead[]; type: "Income" | "Expense" }) => {
      ...
      return (
        <div style={{ marginBottom: "1.25rem", borderRadius: "0.625rem", overflow: "hidden", border }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 0.75rem", background: bg, borderBottom: border }}>
            <span style={{ fontSize: "0.68rem", fontWeight: 700, color, textTransform: "uppercase", letterSpacing: "0.07em" }}>
              {type}
            </span>
            <span style={{ fontSize: "0.68rem", color: C.fgMuted, background: C.surfaceEl, border: `1px solid ${C.border}`, borderRadius: "999px", padding: "0.05rem 0.45rem" }}>
              {items.length}
            </span>
          </div>
    ...
    return (
      <Shell maxWidth="xl" onBackdropDown={handleMouseDown} onBackdropUp={handleMouseUp}>
        <ModalHeader
            ...
            {(["all", "Income", "Expense"] as const).map(f => {
              ...
              return (
                <button
                  ...
                  style={{
                    padding: "0.2rem 0.65rem", borderRadius: "0.35rem", border: "none", cursor: "pointer",
                    fontSize: "0.72rem", fontWeight: 600, transition: "background 0.15s, color 0.15s",
                    background: isActive
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "1.25rem 1.5rem" }}>
          ...
          {!loading && categories.length === 0 && <p style={{ color: C.fgMuted, textAlign: "center", padding: "2rem" }}>No categories found.</p>}
  ...
  if (step === "confirmAdd") return (
    <Shell onBackdropDown={handleMouseDown} onBackdropUp={handleMouseUp}>
      ...
      <div style={{ padding: "1rem 1.5rem", borderTop: `1px solid ${C.border}`, flexShrink: 0 }}>
        <button onClick={handleConfirmAdd} style={{ width: "100%", padding: "0.6rem", borderRadius: "0.5rem", border: "none", background: C.primary, color: "#fff", fontSize: "0.875rem", fontWeight: 600, cursor: "pointer" }}>
          Confirm & Save
        </button>
      </div>
    </Shell>
  ...
  if (step === "deleteConfirm" && selectedCategory && !showUsageCheck) return (
    <Shell onBackdropDown={handleMouseDown} onBackdropUp={handleMouseUp}>
      <ModalHeader title="Delete Category" subtitle="Review before deleting" icon={Trash2} iconColor={C.expense} onClose={onClose} onBack={handleBack} />
      <div style={{ padding: "1.5rem", overflowY: "auto", flex: 1 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", background: "hsl(0 72% 51% / 0.08)", border: `1px solid ${C.expense}50`, borderRadius: "0.5rem", padding: "0.75rem 1rem", marginBottom: "1.25rem" }}>
        ...
        <div style={{ background: C.surfaceEl, border: `1px solid ${C.border}`, borderRadius: "0.625rem", padding: "1rem" }}>
          ...
          <InfoRow label="Type"        value={selectedCategory.type} color={selectedCategory.type === "Income" ? C.income : C.expense} />
          <InfoRow label="Description" value={selectedCategory.description || "—"} />
        </div>
      </div>
      <div style={{ padding: "1rem 1.5rem", borderTop: `1px solid ${C.border}`, display: "flex", gap: "0.75rem", flexShrink: 0 }}>
        <button onClick={handleBack} style={{ flex: 1, padding: "0.6rem", borderRadius: "0.5rem", border: `1px solid ${C.border}`, background: "transparent", color: C.fgMuted, fontSize: "0.875rem", fontWeight: 500, cursor: "pointer" }}>
          Cancel
        </button>
        <button onClick={handleDelete} disabled={deleteLoading} style={{ flex: 1, padding: "0.6rem", borderRadius: "0.5rem", border: `1px solid ${C.expense}`, background: "hsl(0 72% 51% / 0.1)", color: C.expense, fontSize: "0.875rem", fontWeight: 600, cursor: deleteLoading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.4rem" }}>
  ...
  if (step === "deleteConfirm" && selectedCategory && showUsageCheck) return (
    <Shell onBackdropDown={handleMouseDown} onBackdropUp={handleMouseUp}>
      <ModalHeader title="Delete Category" subtitle={selectedCategory.name} icon={Trash2} iconColor={C.expense} onClose={onClose} onBack={handleBack} />
      <div style={{ padding: "1.5rem", overflowY: "auto", flex: 1 }}>
        {transactionUsageCount && transactionUsageCount > 0 ? (
          <div style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", background: "hsl(45 85% 50% / 0.08)", border: "1px solid hsl(45 85% 50% / 0.4)", borderRadius: "0.5rem", padding: "0.75rem 1rem", marginBottom: "1.25rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", background: "hsl(160 60% 45% / 0.08)", border: `1px solid ${C.income}40`, borderRadius: "0.5rem", padding: "0.75rem 1rem", marginBottom: "1.25rem" }}>
            <CheckCircle style={{ width: "1rem", height: "1rem", color: C.income, flexShrink: 0 }} />
            <p style={{ color: C.income, fontSize: "0.82rem", margin: 0, fontWeight: 600 }}>No transactions use this category. Safe to delete.</p>
          </div>
        )}
        <div style={{ background: C.surfaceEl, border: `1px solid ${C.border}`, borderRadius: "0.625rem", padding: "1rem" }}>
          <InfoRow label="Name" value={selectedCategory.name} />
