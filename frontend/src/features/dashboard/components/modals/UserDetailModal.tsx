...
// ── HOISTED: RoleBadge ────────────────────────────────────────────────────────
function getRoleName(u: ReadUserWithCount) {
...
function RoleBadge({ u }: { u: ReadUserWithCount }) {
  ...
  const bg    = isSA  ? "hsl(45 85% 50% / 0.12)"
              : isAdm ? "hsl(160 60% 45% / 0.12)"
              :         "hsl(199 89% 38% / 0.12)";
  return (
    <span style={{ display: "inline-block", padding: "0.15rem 0.55rem", borderRadius: "999px", fontSize: "0.68rem", fontWeight: 700, backgroundColor: bg, color, border: `1px solid ${color}40` }}>
      {getRoleName(u)}
    </span>
  );
}
...
// ── HOISTED: SectionHeading ───────────────────────────────────────────────────
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <p style={{ fontSize: "0.68rem", fontWeight: 700, color: C.fgMuted, textTransform: "uppercase", letterSpacing: "0.08em", margin: "1.25rem 0 0.25rem" }}>
      {children}
    </p>
  );
}
...
export default function UserDetailsModal({ onClose }: OnCloseProps) {
  ...
  // ── Countdown for soft delete ─────────────────────────────────────────────
  useEffect(() => {
    if (!countdownActive || actionType !== "delete") return;
    if (deleteCountdown <= 0) { setCountdownActive(false); return; }
    const timer = setTimeout(() => setDeleteCountdown(p => p - 1), 1000);
  ...
  const handleSoftDelete = async () => {
    ...
    try {
      ...
      const patch = (u: ReadUserWithCount) =>
    ...
    <ShellTable onBackdropDown={handleMouseDown} onBackdropUp={handleMouseUp}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          {selectedUser && (
            <button
              ...
              style={{ background: C.surfaceEl, border: `1px solid ${C.border}`, borderRadius: "0.5rem", color: C.fgMuted, cursor: "pointer", padding: "0.3rem 0.6rem", display: "flex", alignItems: "center", gap: "0.3rem", fontSize: "0.75rem", transition: "border-color 0.15s, color 0.15s" }}
        ...
        {!loading && !selectedUser && (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem", tableLayout: "fixed" }}>
            ...
            <tbody>
              {users.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ padding: "2rem", textAlign: "center", color: C.fgMuted }}>No users found.</td>
                </tr>
              ) : users.map((u, idx) => {
                ...
                return (
                  <tr
                    key={u.id}
                    style={{ backgroundColor: hoveredRow === idx ? C.surfaceHov : isEven ? "transparent" : "hsl(220,14%,14%)", cursor: "pointer", transition: "background-color 0.1s" }}
        ...
        {!loading && selectedUser && (
          <div style={{ padding: "1.5rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.5rem" }}>
              <div style={{ width: "3rem", height: "3rem", borderRadius: "50%", backgroundColor: C.surfaceEl, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <UserCircle style={{ width: "1.6rem", height: "1.6rem", color: C.primary }} />
              </div>
              <div>
                <p style={{ color: C.fg, fontSize: "1rem", fontWeight: 700, margin: 0 }}>{fullName(selectedUser) || "—"}</p>
                <p style={{ color: C.fgMuted, fontSize: "0.75rem", margin: "0.1rem 0 0" }}>{selectedUser.email}</p>
              </div>
            ...
            <DetailRow label="Account Status" value={<ActiveBadge active={selectedUser.is_active} />} />
            <DetailRow label="Created At"     value={new Date(selectedUser.created_at).toLocaleString()} accent={C.fgMuted} />

            <SectionHeading>Activity</SectionHeading>
            <DetailRow label="Transaction Count" value={selectedUser.transaction_count ?? 0} accent={C.primary} />
              ...
              <div style={{ marginTop: "1.5rem" }}>
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: selectedUser.is_active ? "hsl(0 72% 51% / 0.15)" : "hsl(160 60% 45% / 0.15)",
      ...
      {!loading && !selectedUser && users.length > 0 && (
        <div style={{ padding: "0.6rem 1.5rem", borderTop: `1px solid ${C.border}`, fontSize: "0.72rem", color: C.fgMuted, flexShrink: 0 }}>
          ...
          {users.length > 15 ? " · scroll to see more" : ""}
      ...
      {showConfirm && selectedUser && (
        ...
        >
          <div
            ...
            style={{ background: C.surface, border: `1px solid ${isDelete ? C.expense + "60" : C.income + "60"}`, borderRadius: "0.875rem", padding: "1.75rem", width: "100%", maxWidth: "400px", boxShadow: "0 24px 48px rgba(0,0,0,0.6)" }}
          >
            {/* Icon */}
            <div style={{ width: "2.75rem", height: "2.75rem", borderRadius: "50%", backgroundColor: isDelete ? "hsl(0 72% 51% / 0.15)" : "hsl(160 60% 45% / 0.15)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1rem" }}>
              {isDelete
            ...
            <h2 style={{ color: C.fg, fontSize: "1rem", fontWeight: 700, textAlign: "center", margin: "0 0 0.5rem" }}>
              {isDelete ? "Confirm Soft Delete" : "Confirm Restore"}
            </h2>
            <p style={{ color: isDelete ? "hsl(0,72%,70%)" : C.fgMuted, fontSize: "0.8rem", textAlign: "center", lineHeight: "1.5", margin: "0 0 1.5rem" }}>
              {isDelete
                ? `This account has ${txCount} transaction${txCount !== 1 ? "s" : ""}. Are you sure you want to soft delete it?`
                : `Restore this deleted account? It has ${txCount} transaction${txCount !== 1 ? "s" : ""}.`
              }
            </p>
            ...
            <div style={{ display: "flex", gap: "0.75rem", justifyContent: "flex-end" }}>
              <button
                onClick={() => setShowConfirm(false)}
                style={{ padding: "0.5rem 1rem", borderRadius: "0.5rem", fontSize: "0.8rem", fontWeight: 600, background: C.surfaceEl, border: `1px solid ${C.border}`, color: C.fgMuted, cursor: "pointer" }}
              >
                Cancel
              </button>
              ...
              {isDelete ? (
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: "0.4rem", padding: "0.5rem 1rem", borderRadius: "0.5rem", fontSize: "0.8rem", fontWeight: 600, background: countdownActive ? "hsl(0 72% 51% / 0.1)" : "hsl(0 72% 51% / 0.2)", border: `1px solid ${C.expense}${countdownActive ? "40" : "80"}`, color: countdownActive ? "hsl(0,72%,60%)" : C.expense, cursor: countdownActive ? "not-allowed" : "pointer", minWidth: "130px", justifyContent: "center" }}
                >
                ...
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: "0.4rem", padding: "0.5rem 1rem", borderRadius: "0.5rem", fontSize: "0.8rem", fontWeight: 600, background: "hsl(160 60% 45% / 0.2)", border: `1px solid ${C.income}80`, color: C.income, cursor: "pointer", justifyContent: "center" }}
                >
                  <RotateCcw style={{ width: "0.8rem", height: "0.8rem" }} /> Confirm Restore
                </button>
              )}
            </div>
          </div>
