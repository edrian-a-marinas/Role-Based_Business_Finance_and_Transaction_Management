...
function FilterDropdown({ value, onChange }: {
  value:    PromoteViewMode;
  onChange: (v: PromoteViewMode) => void;
}) {
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ color: C.fg, fontSize: "0.85rem", fontWeight: 600, margin: "0 0 0.2rem" }}>
            {fullName || "—"}
          </p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <span style={{
              ...
              fontSize:        "0.68rem",
              fontWeight:      700,
              backgroundColor: target.role_id === 1 ? "hsl(160 60% 45% / 0.12)" : "hsl(199 89% 38% / 0.12)",
              color:           target.role_id === 1 ? C.income : C.primary,
              border:          `1px solid ${target.role_id === 1 ? C.income : C.primary}40`,
            }}>
              {currentRole}
            </span>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              fontSize:       "0.8rem",
              fontWeight:     600,
              background:     canConfirm ? accentBg : "transparent",
              border:         `1px solid ${canConfirm ? accentColor + "80" : C.border}`,
              color:          canConfirm ? accentColor : C.fgMuted,
          ...
          >
            {isPromoting
              ? <TrendingUp   style={{ width: "0.8rem", height: "0.8rem" }} />
              : <TrendingDown style={{ width: "0.8rem", height: "0.8rem" }} />
            }
            {canConfirm
              ? `Confirm ${isPromoting ? "Promote" : "Demote"}`
              : `Wait ${countdown}s…`
            }
          </button>
        </div>
...
export default function PromoteUserModal({ onClose }: OnCloseProps) {
  ...
  // Reset + start countdown whenever a new confirm dialog opens
  useEffect(() => {
  ...
  // Step 1 — just opens the confirm dialog
  ...
  // Step 2 — called only after confirmation
  const handleRoleChange = async () => {
    if (!confirmTarget || confirmNewRole === null) return;
    const targetUserId = confirmTarget.id;
    const newRole      = confirmNewRole;
    const fullName     = [confirmTarget.first_name, confirmTarget.middle_name, confirmTarget.last_name]
      .filter(Boolean).join(" ");
    const newRoleName  = newRole === 1 ? "Admin" : "Standard";
    ...
    try {
      ...
      await api.put(`api/users/${targetUserId}/role`, payload, {
        headers: { Authorization: `${tokenType} ${token}` },
      });
      setUsers((prev) =>
        prev.map((u) => u.id === targetUserId ? { ...u, role_id: newRole } : u)
      );
      setToastMsg(
        `${fullName || "User"} has been ${newRole === 1 ? "promoted to" : "demoted to"} ${newRoleName}.`
      );
    } catch (err) {
  ...
  return (
    <>
                    ...
                    >
                      ...
                      <td style={td}>
                        <span style={{
                          ...
                          backgroundColor: isSA
                            ...
                            ? "hsl(160 60% 45% / 0.12)"
                            : "hsl(199 89% 38% / 0.12)",
                          color:  isSA ? C.warning : isAdm ? C.income : C.primary,
                          border: `1px solid ${isSA ? C.warning : isAdm ? C.income : C.primary}40`,
                        }}>
                          {getRoleName(u)}
                        </span>
                      </td>
                      ...
                      <td style={td}>
                        {isSA ? (
                          ...
                          >
                            <TrendingDown style={{ width: "0.7rem", height: "0.7rem" }} />
                            Demote
                          </button>
                        ) : (
                          <button
                            onClick={() => requestRoleChange(u, 1)}
                            style={{
                              display:      "flex",
                              alignItems:   "center",
                              gap:          "0.3rem",
      ...
      {/* ── Confirm dialog (layered above Shell) ─────────────────────────────── */}
      {confirmTarget && confirmNewRole !== null && (
        <ConfirmDialog
          target={confirmTarget}
          newRole={confirmNewRole}
          countdown={countdown}
